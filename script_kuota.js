...
if (type == "Smartfren") {
    fetch('./data_kuota.json')
        ...
        .then(data => {
            // Get Categories
            for (let i = 0; i < data.smartfren.categories.length; i++) {
                const category = document.createElement("p");

                category.classList.add('category');
                category.classList.add('type' + i);
                if (i == 0) {category.classList.add('active');}
                const category_event = document.createAttribute("onclick");
                category_event.value = "category_1" + i + "()";
                category.setAttributeNode(category_event);

                category.textContent = data.smartfren.categories[i];

                document.getElementById('list-categories').appendChild(category);
            }
    ...
    fetch('./data_kuota.json')
        ...
        .then(data => {
            // Get Categories
            for (let i = 0; i < data.indosatooredoo.categories.length; i++) {
                const category = document.createElement("p");
                ...
                if (i == 0) {category.classList.add('active');}
                const category_event = document.createAttribute("onclick");
                category_event.value = "category_2" + i + "()";
                category.setAttributeNode(category_event);

                category.textContent = data.indosatooredoo.categories[i];
    ...
    fetch('./data_kuota.json')
        ...
        .then(data => {
            // Get Categories
            for (let i = 0; i < data.xl.categories.length; i++) {
                const category = document.createElement("p");
                ...
                if (i == 0) {category.classList.add('active');}
                const category_event = document.createAttribute("onclick");
                category_event.value = "category_3" + i + "()";
                category.setAttributeNode(category_event);

                category.textContent = data.xl.categories[i];
    ...
    fetch('./data_kuota.json')
        ...
        .then(data => {
            // Get Categories
            for (let i = 0; i < data.telkomsel.categories.length; i++) {
                const category = document.createElement("p");
                ...
                if (i == 0) {category.classList.add('active');}
                const category_event = document.createAttribute("onclick");
                category_event.value = "category_4" + i + "()";
                category.setAttributeNode(category_event);

                category.textContent = data.telkomsel.categories[i];
...
function pesan(i) {
    ...
    price = document.querySelector('.item' + i + ' p:nth-child(5)').textContent;
