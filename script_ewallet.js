...
document.querySelector('.ewallet-type').textContent = type;


// Create Elements
if (type == "Shopee Pay") {
    fetch('./data_ewallet.json')
        .then(response => response.json())
        .then(data => {
            var products = [];
            for (let i = 0; i < data.shopeepay.length; i++) {
                ...
                text1.textContent = data.shopeepay[i].description;
                text2.textContent = "Harga";
                text3.textContent = data.shopeepay[i].price;
                ...
                products.push(data.shopeepay[i].product);
            }
    ...
    fetch('./data_ewallet.json')
        ...
        .then(data => {
            var products = [];
            for (let i = 0; i < data.dana.length; i++) {
                const card_item = document.createElement("div");
                const text1 = document.createElement("p");
                const text2 = document.createElement("p");
                const text3 = document.createElement("p");
                ...
                text1.textContent = data.dana[i].description;
                text2.textContent = "Harga";
                text3.textContent = data.dana[i].price;
                ...
                products.push(data.dana[i].product);
            }
            localStorage.removeItem('product_code');
            localStorage.setItem('product_code', JSON.stringify(products));
        })
        .catch(err => console.error(err));
} else if (type == "Link Aja") {
    fetch('./data_ewallet.json')
        .then(response => response.json())
        .then(data => {
            var products = [];
            for (let i = 0; i < data.linkaja.length; i++) {
                const card_item = document.createElement("div");
                const text1 = document.createElement("p");
                const text2 = document.createElement("p");
                const text3 = document.createElement("p");
                ...
                text1.textContent = data.linkaja[i].description;
                text2.textContent = "Harga";
                text3.textContent = data.linkaja[i].price;
                ...
                products.push(data.linkaja[i].product);
            }
            localStorage.removeItem('product_code');
            localStorage.setItem('product_code', JSON.stringify(products));
        })
...
function pesan(i) {
    ...
    document.querySelector('#ewallet').textContent = type;
